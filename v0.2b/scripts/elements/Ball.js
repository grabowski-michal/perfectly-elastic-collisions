    ...
    checkCollision (object) {
        ...
        for (var s = 0; s < this.sectionIndexes.length; s++) {
            for (var i = 0; i < time.space.sections[this.sectionIndexes[s]].elements.length; i++) {
                if (time.space.sections[this.sectionIndexes[s]].elements[i] != this) {
                    switch (time.space.sections[this.sectionIndexes[s]].elements[i].shape) {
                        case "round":
                            var bp1 = this.position;
                            var br1 = this.radius;
                            var bv1 = this.velocity;
                            var bm1 = this.mass;
                            var bp2 = time.space.sections[this.sectionIndexes[s]].elements[i].position;
                            var br2 = time.space.sections[this.sectionIndexes[s]].elements[i].radius;
                            var bv2 = time.space.sections[this.sectionIndexes[s]].elements[i].velocity;
                            var bm2 = time.space.sections[this.sectionIndexes[s]].elements[i].mass;

                            var V = new Vector2D(bp2.x - bp1.x, bp2.y - bp1.y);
                            var distance = V.getLength() - br1 - br2;

                            if (distance <= 0) {
                                if (time.space.sections[this.sectionIndexes[s]].elements[i] != this.lastCollisionObject || time.space.sections[this.sectionIndexes[s]].elements[i].lastCollisionObject != this) {
                                    //this.inCollision = true;
                                    this.lastCollisionObject = time.space.sections[this.sectionIndexes[s]].elements[i];
                                    time.space.sections[this.sectionIndexes[s]].elements[i].lastCollisionObject = this;
                                    ...
                                    time.space.sections[this.sectionIndexes[s]].elements[i].velocity = new Vector2D(Math.cos(collisionAngle) * finalVel2.x + Math.cos(collisionAngle + Math.PI/2) * finalVel2.y,
                        ...
                        case "rectangular":
                            var bp1 = this.position;
                            var br1 = this.radius;
                            var rp1 = time.space.sections[this.sectionIndexes[s]].elements[i].position;
                            var rw1 = time.space.sections[this.sectionIndexes[s]].elements[i].width;
                            var rh1 = time.space.sections[this.sectionIndexes[s]].elements[i].height;
                            var collisionPoint;
