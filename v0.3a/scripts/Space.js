class Space {
    ...
    evalAllTheElements () {
        ...
        this.frequencyFactor = parseFloat(Parameters.DEFAULT_MAX_VELOCITY) + 1;
        var temporaryFrequencyFactor = this.frequencyFactor;
        for (var i = 0; i < this.elements.length; i++) {
            this.elements[i].evalPosition(temporaryFrequencyFactor);
        }
    }
    ...
    initiateSections (ballRadius, canvasWidth, canvasHeight) {
        ...
        for (var k = 0; k < canvasHeight; k += heightOfSection) {
            for (var j = 0; j < canvasWidth; j += widthOfSection) {
                var section = {
                    leftPos: j - ballRadius,
                    rightPos: j + widthOfSection + ballRadius,
                    topPos: k - ballRadius,
                    bottomPos: k + heightOfSection + ballRadius,
